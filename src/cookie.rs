//! A minimal RFC 6265 cookie jar (feature `cookies`, client-only): parses
//! `Set-Cookie` response headers, stores them scoped by domain/path, and
//! builds the `Cookie` request header for subsequent requests to the
//! same origin. Ported near-verbatim from `rusty_request`'s `cookie.rs`
//! (donor 3 of the mission handoff) -- [`CookieJar`] is `pub` here
//! (donor's was `pub(crate)`, since only its own `Client` used it) and
//! keys on [`crate::Url`] instead of a client-local type.
//!
...
//! that is a suffix of the host that actually sent it. This is a real,
//! pre-existing limitation, not introduced by this port -- callers
//! relying on this jar for security-sensitive scoping across
//! multi-tenant domains (`*.github.io`-style hosting) should know it
//! doesn't defend against that case.
...
/// A client-side cookie store, scoped and expired per RFC 6265.
...
impl CookieJar {
    ...
    /// match: domain/path match, not expired, and never a `Secure`
    /// cookie unless `url`'s scheme is `https`. Also prunes expired
    /// entries from the jar as a side effect.
    pub fn cookie_header_for(&mut self, url: &Url) -> Option<String> {

...
impl Curve {
    ...
    /// Double-and-add over the big-endian scalar bytes, matching Go's
    /// `crypto/elliptic` semantics (the group has prime order, so reduction
    /// differences cannot change the result).

...
//! compatible.
//!
//! Two request types for now — `Produce` (append) and `Fetch` (read one
//! record back) — matching Phase 1's actual storage surface
//! (`crate::retention::Log`/`crate::Segment`). Encoding is deliberately pure
//! and synchronous, the same reason `crate::record`'s framing has no I/O in
//! it: testable without any runtime at all.
//!
...
pub enum ProtocolError {
    ...
    UnknownTag(u8),
    /// An [`Response::Error`] message's bytes weren't valid UTF-8. Never
    /// applies to `Produce`/`Fetch` payloads — those are opaque bytes and
    /// don't need to be.
    InvalidUtf8,
}

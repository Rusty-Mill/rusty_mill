...
//!
//! Needed for the CredSSP (MS-CSSP) version 5+ public-key binding, which hashes
//! a nonce together with the server's TLS public key. Unlike the other hashes
//! in this crate SHA-256 is not broken, but it is still implemented by hand to
...
pub fn sha256(data: &[u8]) -> [u8; SHA256_DIGEST_LEN] {
    ...
    let pad_to = if rest.len() + 1 + 8 <= 64 { 64 } else { 128 };
    tail[pad_to - 8..pad_to].copy_from_slice(&bit_len.to_be_bytes());
...
mod tests {
    ...
    #[test]
    fn multi_block() {
        // 1,000,000 'a' — the classic long FIPS vector.

            ...
            match event {
                ...
                Event::MousePressed(MouseButton::Left) => {
                    if btn_record.contains(cursor_pos.0, cursor_pos.1) {
                        ...
                        if let Some(ref mut cap) = audio_capture {
                            let samples = cap.read_samples();
                            sample_count += samples.len();
                        }
                        transcription_text = format!("RECORDING AUDIO: {} SAMPLES", sample_count);
                        ...
                        transcription_text = String::from("TRANSCRIPTION COMPLETE: SOVEREIGN RUSTY MILL VOICE STUDIO");
                        println!("🔵 [UI Interaction] TRANSCRIBE clicked!");

...
| [0002](0002-bundled-shell-rush.md) | Bundled shell (`rush`) | Accepted — Stage 1 landed | **Incorporated, staged & opt-in.** Vendored `baileyrd/rush` as the `nexus-rush` library crate; `nexus-terminal` runs the bundled shell for *sandboxed* sessions (opt-in via `sandbox.toml`); system shell stays the default. |
| [0003](0003-terminal-emulator-rusty-term.md) | `rusty_term` terminal emulator | Accepted — Tracks A+B landed | **Selectively adopted.** Not the GUI (ADR 0011); adopted the headless VT grid core as `nexus-vt`, OSC 133 command/exit-code capture in `nexus-terminal`, and terminal-as-MCP-resource introspection in `nexus-mcp`. |
...
| [0009](0009-four-repo-consolidation.md) | Four-repo consolidation (`Rustsidian`, `nexus_forge`, `rusty_nexus`) | Draft — assessment | **Consolidate onto `nexus`; harvest, don't merge.** The siblings are earlier (or one later spike) attempts at the same product; port four small verified gaps from `nexus_forge` (unique note, random note, note composer, typed properties) into existing crates/shell plugins, copy Rustsidian's research docs, archive all three repos. |
...
[`../memory.md`](../memory.md)). The three product-level siblings (`Rustsidian`,
`nexus_forge`, `rusty_nexus`) are assessed together in RFC 0009.

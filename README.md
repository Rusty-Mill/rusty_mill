...
| [`rusty-hister-model`](crates/rusty_hister/crates/rusty-hister-model) | `crates/rusty_hister/crates/rusty-hister-model` | rusty_hister: persisted schema on `rusty_db`, dual SQLite/Postgres (schema + embedding-queue and `WebSession` query layers implemented; other models' query-layer helpers not yet started) |

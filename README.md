...
| [`rusty-hister-model`](crates/rusty_hister/crates/rusty-hister-model) | `crates/rusty_hister/crates/rusty-hister-model` | rusty_hister: persisted schema on `rusty_db`, dual SQLite/Postgres (schema + embedding-queue, `WebSession`, `DocumentVersion`, and `crawl.go` (`CrawlJob`+`CrawlURL`) query layers implemented; `history.go`/`user.go` query helpers not yet started) |
| [`rusty-hister-extractor`](crates/rusty_hister/crates/rusty-hister-extractor) | `crates/rusty_hister/crates/rusty-hister-extractor` | rusty_hister: extractor chain-of-responsibility registry (no concrete extractors yet) |

...
- **RemoteFX / GFX codec support.** The channel plumbing (`vchan`, `dvc`,
  ...
  and chroma subsampling, decode straight to RGBA. Still needed: the
  remaining GFX codecs (AVC420/444, ClearCodec) — `gfx` still carries their
  `bitmapData` as opaque bytes for those.

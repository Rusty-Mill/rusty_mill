...
| REG-140 | `SchemaRegistryEnforcer.DEFAULT_COMPATIBILITY = CompatibilityMode.FULL_TRANSITIVE` | config | code | — | REQUIRED | | |
| REG-141 | `SchemaRegistryEnforcer(url, client=None)` constructs its own `SchemaRegistryClient({"url": url})` when no client injected | config | code | — | REQUIRED | | |
| REG-142 | `initialize_global_compatibility()` calls `client.set_compatibility("FULL_TRANSITIVE")` with no subject_name (registry-wide default) | behavior | code+test | — | REQUIRED | | |
| REG-143 | `set_subject_compatibility(subject, mode)` accepts `CompatibilityMode` enum or coerces raw string | behavior | code+test | — | REQUIRED | | |
| REG-144 | `set_subject_compatibility` raises `ValueError` (listing valid modes) for unrecognized string, before calling underlying client | behavior | code+test | — | REQUIRED | | |
| REG-145 | `get_subject_compatibility(subject)` delegates to `client.get_compatibility(subject_name=subject)` | interface | code+test | — | REQUIRED | | |
| REG-146 | `register_schema(subject, schema_str)` wraps into `Schema` object with `"AVRO"` type before `client.register_schema` | behavior | code+test | — | REQUIRED | | |
| REG-147 | `register_schema` returns integer schema id from underlying client on success | interface | code+test | — | REQUIRED | | |
| REG-148 | `register_schema` catches `SchemaRegistryError` with `error_code==409`, raises `CompatibilityViolation(subject, message)` instead | behavior | code+test | — | REQUIRED | | |
| REG-149 | `register_schema` re-raises `SchemaRegistryError` unchanged for any other error_code | behavior | code+test | — | REQUIRED | | |
| REG-150 | `CompatibilityMode` enum: 7 members — BACKWARD, BACKWARD_TRANSITIVE, FORWARD, FORWARD_TRANSITIVE, FULL, FULL_TRANSITIVE, NONE | interface | code | — | REQUIRED | | |
| REG-151 | `CompatibilityViolation.__str__` formats as `"Schema incompatible with {subject}: {message}"` | behavior | code+test | — | REQUIRED | | |
| REG-152 | `CompatibilityViolation` exposes `.subject`/`.message` attributes independently | behavior | code+test | — | REQUIRED | | |
...
| SDK-009 | `OutputPortSpec` immutable (`@dataclass(frozen=True)`): `name`, `topic`, `event_type: type[BaseEvent]`, `event_classification: EventType` | interface | code+test | — | REQUIRED | | |
| SDK-010 | `OutputPortSpec` mutation after construction raises `dataclasses.FrozenInstanceError` | behavior | code+test | — | REQUIRED | | |
| SDK-011 | `ContractVersionMismatch(expected, actual)` stores `.expected`/`.actual`, stringifies `"Contract version mismatch: expected {expected!r}, got {actual!r}"` | interface | code | — | REQUIRED | | |
| SDK-012 | `RegistryError(message)` stores `.message`; raised by `RegistryClient` on any non-2xx (except `get_contract`'s 404) | interface | code | — | REQUIRED | | |
...
| SDK-065 | `TopicType(str, Enum)`: STATE, EVENTS, COMMANDS, DLQ | interface | code | — | REQUIRED | | |
| SDK-066 | `TopicSpec` dataclass: `name`, `topic_type` (required), `num_partitions=3`, `replication_factor=1`, `retention_ms=2_592_000_000` | config | code+test | — | REQUIRED | | |
| SDK-067 | `TopicSpec.kafka_config()` returns compact-policy config (`cleanup.policy=compact`, `min.cleanable.dirty.ratio=0.1`, `segment.ms=86400000`) for STATE type | behavior | code+test | — | REQUIRED | | |
| SDK-068 | `TopicSpec.kafka_config()` returns `{cleanup.policy:delete, retention.ms:str(retention_ms)}` for non-STATE types | behavior | code+test | — | REQUIRED | | |
| SDK-069 | Documented invariant: no code outside `topic_config.py` should build raw Kafka config dicts | docs | docs | — | REQUIRED | | |
| SDK-070 | `TOPIC_PATTERN` regex `^[a-z][a-z0-9-]*\.[a-z][a-z0-9-]*\.[a-z][a-z0-9-]*$` — `{domain}.{product}.{stream-type}`, 3 segments | config | code+test | — | REQUIRED | | |
| SDK-071 | `TopicNameError(ValueError)` raised by `_validate_name()` on pattern mismatch; message includes "violates convention" | behavior | code+test | — | REQUIRED | | |
| SDK-072 | `TopicManager(admin_client)` takes constructed `AdminClient` via DI; docstring: `create_topics()` must only be called from within `TopicManager` | interface | code+docs | — | REQUIRED | | |
| SDK-073 | `create_topic(spec)` validates name (raising `TopicNameError`) before any Kafka call | behavior | code+test | — | REQUIRED | | |
| SDK-074 | `create_topic(spec)` builds `NewTopic(...)`, calls `admin_client.create_topics([...])`, calls `.result()` on each future — synchronous blocking, re-raises `KafkaException` | behavior | code+test | — | REQUIRED | | |
| SDK-075 | `create_topic(spec)` does NOT itself swallow "already exists" — that's handled one layer up in `DataProductProducerBase.startup()` | behavior | code+test | — | REQUIRED | | |
| SDK-076 | On success, `create_topic` stores spec in in-memory `self._registry: dict[str,TopicSpec]`, not persisted | behavior | code | — | REQUIRED | | |
| SDK-077 | `deprecate_topic(name)` records `self._deprecated[name]=now`; no Kafka API call, no delete/alter, no verification the name was created via this instance | behavior | code+test | — | REQUIRED | | |
| SDK-078 | `list_topics()` returns one dict per entry in `self._registry` (topics not created via this instance never appear): `name`, `topic_type`, `deprecated: bool`, `deprecated_at` | interface | code+test | — | REQUIRED | | |
| SDK-079 | Documented invariant: no code outside `topic_manager.py` should call `AdminClient.create_topics()` directly | docs | docs | — | REQUIRED | | |
...
| GOV-001 | `GovernanceEngine.evaluate()` runs all registered policy callables against a `DataProductCreate` in registration order; a policy "passes" simply by returning `None` (no raise) | behavior | code+test | — | REQUIRED | | |
| GOV-002 | `GovernanceEngine.evaluate()` catches `ValueError`/`AssertionError` raised by a policy and appends `str(exc)` verbatim as the violation string (no prefix/wrapping) | behavior | code+test | — | REQUIRED | | |
| GOV-003 | `GovernanceEngine.evaluate()` catches any other `Exception` subtype from a policy and wraps it as `"[POLICY-ERROR] {policy.__name__}: {exc}"` instead of the raw message | behavior | code+test | — | REQUIRED | | |
| GOV-004 | `GovernanceEngine.register_policy(policy)` appends a callable to the internal policy list at runtime; it is evaluated starting on the *next* `evaluate()` call | interface | code+test | — | REQUIRED | | |
| GOV-005 | `GovernanceEngine.__init__(policies=None)` accepts an optional initial list of `PolicyFn`; `None`/falsy defaults to an empty list (copied via `list(policies)`, not aliased) | interface | code+test | — | REQUIRED | | |
| GOV-006 | `require_description_min_length` raises `ValueError` when `description` is `None` or shorter than 20 characters; message reports the actual measured length | behavior | code+test | — | REQUIRED | | |
| GOV-007 | `require_semantic_version` raises `ValueError` unless `version` fully matches `^\d+\.\d+\.\d+$` (rejects `"v1.0"`, 2-part versions, pre-release/build suffixes, `None`) | behavior | code+test | — | REQUIRED | | |
| GOV-008 | `require_domain_lowercase` raises `ValueError` when `domain` contains any uppercase character (`domain != domain.lower()`) | behavior | code+test | — | REQUIRED | | |
| GOV-009 | Module-level `_DEFAULT_ENGINE` singleton is pre-loaded with exactly the 3 built-in policies, in this fixed order: description-length, semver, domain-lowercase | config | code | — | REQUIRED | | |
...
| XFM-041 | `PlatformConfig.kafka_bootstrap_servers: str`, default `"localhost:9092"`, env `MESHED_KAFKA_BOOTSTRAP_SERVERS` | config | code | — | REQUIRED | | |
| XFM-042 | `PlatformConfig.schema_registry_url: str`, default `"http://localhost:8081"`, env `MESHED_SCHEMA_REGISTRY_URL`, documented convention "no trailing slash" (not enforced in code) | config | code | — | REQUIRED | | |
| XFM-043 | `PlatformConfig.default_num_partitions: int`, default `3`, validation `ge=1`, env `MESHED_DEFAULT_NUM_PARTITIONS` | config | code | — | REQUIRED | | |
| XFM-044 | `PlatformConfig.default_replication_factor: int`, default `1`, validation `ge=1`, env `MESHED_DEFAULT_REPLICATION_FACTOR` | config | code | — | REQUIRED | | |
| XFM-045 | `PlatformConfig.default_retention_ms: int`, default `2_592_000_000` (30 days), validation `ge=1`, env `MESHED_DEFAULT_RETENTION_MS` | config | code | — | REQUIRED | | |
| XFM-046 | `PlatformConfig.registry_db_path: str`, default `"meshed_registry.db"`, env `MESHED_REGISTRY_DB_PATH` | config | code+test | — | REQUIRED | | |
| XFM-047 | `PlatformConfig.registry_base_url: str`, default `"http://localhost:8000"`, env `MESHED_REGISTRY_BASE_URL` | config | code | — | REQUIRED | | |
| XFM-048 | `PlatformConfig` global `env_prefix="MESHED_"` applied to every field via `SettingsConfigDict(env_prefix="MESHED_")` | config | code | — | REQUIRED | | |

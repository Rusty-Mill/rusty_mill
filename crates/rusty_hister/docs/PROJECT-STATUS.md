...
| `rusty-hister-extractor` | **Implemented — all 20 built-in extractors done** — `Registry` (chain-of-responsibility: ordered registration, two-phase enrich/extract, preview-chain starting points, config merging), plus `JsonLdExtractor` (§4.5.5), `EmbeddedVideoExtractor` (§4.5.3), `StackExchangeExtractor` (§4.5.7), `GoDocExtractor` (§4.5.8), `LobstersExtractor` (§4.5.10), `HackerNewsExtractor` (§4.5.11), `GitHubExtractor` (§4.5.9), `ChatGptExtractor` (§4.5.18), `BasicExtractor` (§4.4), `WikipediaExtractor` (§4.5.12), `RedditExtractor` (§4.5.6), `DiscourseExtractor` (§4.5.4), `YtdlpExtractor` (§4.5.17), `MarkdownExtractor` (§4.5.1), `OrgModeExtractor` (§4.5.2), `NotionExtractor` (§4.5.16), `ReadabilityExtractor` (§4.4), `MastodonExtractor` (§4.5.13), `BlueskyExtractor` (§4.5.14), and `TwitterExtractor` (§4.5.15) as concrete extractors, and shared support modules (`sanitizer`, `urlutil`, `textutil`) other extractors reuse. 204 unit tests, clippy/fmt clean. `Document::extra_documents`/`skip_indexing` (added for Mastodon) ended up shared by all three social-media extractors (Mastodon, Bluesky, Twitter). |

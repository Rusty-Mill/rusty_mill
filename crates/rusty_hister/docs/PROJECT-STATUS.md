...
or hand-rolling the algorithm. Mastodon/Bluesky/Twitter flagged as
blocked on a `rusty-hister-core` capability gap — Go's per-page
multi-document extraction — rather than silently ported without it).
...
| `rusty-hister-extractor` | **In progress** — `Registry` (chain-of-responsibility: ordered registration, two-phase enrich/extract, preview-chain starting points, config merging), plus `JsonLdExtractor` (§4.5.5), `EmbeddedVideoExtractor` (§4.5.3), `StackExchangeExtractor` (§4.5.7), `GoDocExtractor` (§4.5.8), `LobstersExtractor` (§4.5.10), `HackerNewsExtractor` (§4.5.11), `GitHubExtractor` (§4.5.9), `ChatGptExtractor` (§4.5.18), `BasicExtractor` (§4.4), `WikipediaExtractor` (§4.5.12), `RedditExtractor` (§4.5.6), and `ReadabilityExtractor` (§4.4) as concrete extractors, and shared support modules (`sanitizer`, `urlutil`, `textutil`) other extractors reuse. 146 unit tests, clippy/fmt clean. `scraper`/`ammonia` now available for the remaining 8 built-in extractors that need real HTML parsing/sanitizing (Mastodon/Bluesky/Twitter additionally blocked on a core capability gap — see Open items). |

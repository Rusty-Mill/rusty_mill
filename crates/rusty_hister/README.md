...
| [`rusty-hister-model`](crates/rusty-hister-model) | `crates/rusty_hister/crates/rusty-hister-model` | Persisted schema on `rusty_db` (dual SQLite/Postgres) — history, links, users, crawl jobs, embedding queue, sessions — **implemented** (Phase 1: schema, plus the embedding-queue and `WebSession` query layers; the other five models' query-layer helpers not yet started) |

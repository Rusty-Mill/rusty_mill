...
impl OffsetCommitResponse {
    ...
    pub fn decode(reader: &mut Reader) -> Result<Self, CodecError> {
        const OFFSET_COMMIT_TOPIC_MIN_LEN: usize = 2 + 4; // empty name + partition_count
        let topic_count = read_checked_array_len(reader, OFFSET_COMMIT_TOPIC_MIN_LEN)?;
        let mut topics = Vec::with_capacity(topic_count);
        for _ in 0..topic_count {
            let name = read_string(reader)?;
            const OFFSET_COMMIT_PARTITION_MIN_LEN: usize = 4 + 2; // partition_index + error_code
            let partition_count = read_checked_array_len(reader, OFFSET_COMMIT_PARTITION_MIN_LEN)?;
...
mod tests {
    ...
    fn decode_rejects_a_huge_topic_count_from_a_tiny_buffer() {
        ...
        let bytes = writer.into_vec();
        let mut reader = Reader::new(&bytes);
        let err = OffsetCommitResponse::decode(&mut reader).unwrap_err();
        assert!(matches!(
            err,

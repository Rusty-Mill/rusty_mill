...
- **Three real corrections, found during implementation, before writing
  the code the Decision above sketched — all narrowing scope, none
...
- Proven: `tests/server_backup_integration.rs` (5 tests, real socket,
  ...
  target green (including `server_python_client`, fixed in a
  follow-up round the same session — see `ADR-0043`'s own final
  addendum).

...
pub enum ErrorCode {
    ...
    Conflict,
    /// Protocol 13 (`INS-FR-007`, ADR-0046). [`Request::Insert`]'s `id`
    /// already has a record; nothing was written. Only ever answers an
    ...
    /// this frame. The one frame ever written before negotiation, on a
    /// plaintext listener only — a client that speaks 30 reads it as
...
pub enum Request {
    ...
    DescribeRelations,
    /// Protocol 13 (`INS-FR-007`, ADR-0046,
    /// `docs/design/SERVER-INSERT-DESIGN.md`): add one record to this
    /// connection's table at runtime — the first request that changes
    /// the record *set* rather than one field of one record. `id` is
    /// minted by the client (the consumer mints its own on every path;
    /// `crate::generic::entity::entity_id` exists for exactly this);
    /// `fields` is [`Response::Record`]'s own shape, so a client reads a
    /// record and writes one with the same code, and must name every
    /// field the schema describes exactly once with a value of its
    /// `ValueKind` — `Malformed` otherwise, `UnknownField` for a tag the
    /// schema doesn't describe; nothing is written on any refusal.
    /// Answered [`Response::Ok`], or `Err { Duplicate }` when `id`
    /// already has a record. `Unsupported` from an adapter that does not
    /// implement insertion. Gated server-side: `Malformed` below 13;
    /// `Unauthorized` for a `ReadOnly` token; `SessionOpen` while a
    /// session is open (never staged, never journaled).
    Insert {
        id: RecordId,
        fields: Vec<(FieldRef, ScanValue)>,
    },
    ...
    /// Protocol 15 (`REP-FR-006`, ADR-0049,
    /// `docs/design/SERVER-REPLACE-DESIGN.md`): replace one record of
    /// this table **whole** — [`Request::Insert`]'s exact shape over an
    /// id that already has a record, the consumer's `update_memory`/
    /// `entity_upsert` write path. `fields` must name every field the
    /// schema describes exactly once with a value of its `ValueKind`
    /// (`Malformed` otherwise, `UnknownField` for a tag the schema
    /// doesn't describe), and the domain's own rules apply as at insert;
    /// nothing is written on any refusal. Answered [`Response::Ok`], or
    /// [`Response::NotFound`] when `id` has no record — `UpdateField`'s
    /// own not-found shape, since both write to an existing record.
    /// Relation edges are not fields: a replaced record keeps every
    /// neighbor, parent, and child it had. `Unsupported` from an adapter
    /// that does not implement replacement. Gated server-side:
    /// `Malformed` below 15; `Unauthorized` for a `ReadOnly` token;
    /// `SessionOpen` while a session is open (never staged, never
    /// journaled).
    Replace {
        id: RecordId,
    ...
    /// Protocol 16 (`TBL-FR-002`, ADR-0045/ADR-0050,
    /// `docs/design/SERVER-TABLES-DESIGN.md`): select which of the
    /// server's tables every following table-less request (`GetById`,
    /// `Query`, `DescribeSchema`, `Insert`, `Join` with `right_table:
    /// None`, …) is served from. Per-connection state, like the
    /// negotiated version and the authenticated class; the default is
    /// the server's primary table, so a connection that never sends
    /// `Use` is byte-for-byte a pre-16 connection. Answered
    /// [`Response::Ok`]; an unknown name is `Malformed`; inside a
    /// session `SessionOpen` (a session's writes belong to one table).
    /// Not a write: a `ReadOnly` token may send it. `Malformed` below 16.
    Use {
        table: String,
    },
    ...
    /// Protocol 20 (`PAG-FR-004`, ADR-0055,
    /// `docs/design/SERVER-PAGE-DESIGN.md`): one **ordered keyset page**
    /// of this table — every record sorted ascending by `order_by` (an
    /// orderable `U32`/`I64` field) with the record id as the tie-break,
    /// starting strictly after `after` (a `(value, id)` cursor; `None`
    /// for the first page), at most `limit` rows, answered with
    /// [`Response::Rows`] carrying every field of each record. The last
    /// row's `(order_by value, id)` is the next page's cursor; a puller
    /// wanting "everything after time *t*" starts at `(t, RecordId::max)`.
    /// Validated before any scan: `UnknownField` for an `order_by` the
    /// schema lacks, `Malformed` for a field that is not `U32`/`I64`, a
    /// cursor value of another kind, or a `limit` of zero. Evaluated over
    /// `ConnectionStore::scan_all` by default (`SQL-FR-004`'s posture — an
    /// adapter may answer it more cheaply). A read, gated exactly as
    /// `Query`: authentication only; never overlaid by a session, never
    /// read-set-tracked. `Malformed` below 20 (rule 3).
    Page {
        order_by: FieldRef,
...
pub enum Response {
    ...
    /// Protocol 16 (`TBL-FR-003`, ADR-0050). Answers
    /// [`Request::ListTables`]: every table's name in registration order,
    /// and the primary's. Only ever answers a gated request, so it needs
    /// no `downgrade_for_version` arm.
    Tables {
        names: Vec<String>,
        primary: String,
    },
...
mod tests {
    ...
    fn every_request_variant_encodes_to_its_pinned_bytes() {
        ...
        assert_golden(
            ...
            &bytes(&[
                &[0x18, 0x00, 0x00, 0x00],                         // Use
                &[0x06, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00], // table len
                b"entity",
            ]),
        );
        assert_golden(
        ...
        // Protocol 15 (`REP-FR-006`, ADR-0049): `Replace` at 23 — `Insert`'s
        // exact body shape.
        assert_golden(
            "Replace",
            &Request::Replace {
    ...
    fn every_response_variant_encodes_to_its_pinned_bytes() {
        ...
        assert_golden_eq(
            ...
            &bytes(&[
                &[0x11, 0x00, 0x00, 0x00],                         // Tables
                &[0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00], // two names
                &[0x06, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00],
                b"memory",
                &[0x06, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00],
                b"entity",
                &[0x06, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00], // primary len
                b"memory",
            ]),
        );
        for (code, index) in [

...
/// `MvccHandle` for the full contract; identical here, except `Dog` has
/// no insert log or `Compact` at all (no runtime `Insert`/`Replace`/
...
impl<S> DogConnectionStore<S> {
    ...
    /// Simplified relative to `Memory`/`Entity`/`Relation`'s own version:
    /// `Dog` has no insert log to fold at open time, so this only reads
    /// the persisted `.mvcc` sidecar (if any) — the "journal remainder
    /// gap" this leaves is the same accepted, pre-existing, shared
    /// limitation named in `docs/design/MVCC-OPEN-HOOK-PROPOSAL.md`'s own
    /// "Open questions" section. Independent of `S`'s own capabilities,
    /// like `with_backup_source`.
    ...
    S::Exclusive: CheckpointFlush,
{
    ...
    /// currently-live record, once) and open a new snapshot. The id list
    /// is read via `crate::production::AllIds::all_ids(&self.store)`
    /// *before* the exclusive section — that trait is `&self`-shaped
    /// (unlike `crate::generic::query::AllIds<R>`, which the
    /// `GenericMmapStore`-based domains use inside `with_exclusive`) and
    /// this crate's own "no runtime deletion" invariant means the id set
    /// itself never changes at runtime, only field values do, so reading
    /// it outside the lock and then re-reading each record's current
    /// value inside the lock is race-free.
    fn mvcc_begin(&self) -> u64 {
        ...
        if !mvcc.state.is_active() {
            ...
            self.store.with_exclusive(|inner| {
                mvcc.state.activate();
                for id in ids {
                    if let Some(record) = DogStore::get(inner, id) {
                        mvcc.state.with_index(|index| {
                            index.record_write(
                            ...
                            index.record_write(
                                (id, FIELD_BREED),
                                mvcc::BASELINE_TXN,
                                Some(ScanValue::Str(record.breed)),
                            );
                            index.record_write(
                                (id, FIELD_AGE),
                                mvcc::BASELINE_TXN,
                                Some(ScanValue::U32(record.age)),
                            );
                        });
                    }
                }
...
mod tests {
    ...
    /// [`DogConnectionStore::with_mvcc`] so `SESSION_MVCC_ISOLATION` is
    /// available — but not yet activated (`mvcc_begin` does that).
    fn sample_adapter_with_mvcc() -> DogConnectionStore<ProductionStore> {
        let dir = fresh_temp_dir("server_dog_mvcc_adapter").unwrap();
        let path = dir.join("dogs.mmap");
        let records = vec![
            DogRecord::new(Uuid::from_u128(1), "labrador", 3),
            DogRecord::new(Uuid::from_u128(2), "labrador", 5),
        ...
        DogConnectionStore::new(store).with_mvcc(&path).unwrap()
    }
    ...
    fn apply_transaction_mvcc_detects_a_write_write_conflict() {
        ...
        assert_eq!(adapter.get(id).unwrap()[1], (FIELD_AGE, ScanValue::U32(10)));

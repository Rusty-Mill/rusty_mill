//! A minimal, real server binary for the `Reminder` domain — wraps a
//! `GenericProductionStore<ReminderProductionStack>` seeded from a small,
//! hand-written sample dataset in a `ReminderConnectionStore` and serves
//! it over TCP. Mirrors `dog_server.rs`'s own shape and env-var reading
//! exactly (`RMD-FR-008`, ADR-0036) — every operational knob `dog_server`
...
//! — see ADR-0010's Consequences. Usage: `reminder_server [host:port]`
//! (defaults to `127.0.0.1:7879`).
//!
...
fn main() {
    ...
    let dir = std::env::temp_dir().join(format!("reminder_server_{}", std::process::id()));
    std::fs::create_dir_all(&dir).expect("creating a scratch directory for the mmap-backed store");
    let path = dir.join("reminders.mmap");

    let store = create_reminder_production_stack(sample_reminders(), &path)
        .expect("creating the sample ReminderProductionStack");
    ...
    let connection_store = Arc::new(match std::env::var("SERVER_TXN_JOURNAL_PATH") {
        Ok(journal_path) => ReminderConnectionStore::with_journal(
        ...
        Err(_) => ReminderConnectionStore::new(GenericProductionStore::new(store)),
    });
    ...
    let options = match std::env::var("SERVER_METRICS_HTTP_ADDR") {
        Ok(addr) => {
            ...
            eprintln!("reminder_server metrics HTTP listening on {addr} (SERVER_METRICS_HTTP_ADDR, ADR-0069)");
            options.with_metrics_http(listener)
        }
        Err(_) => options,
    };
    eprintln!(
        "reminder_server listening on {addr} (auth: {}, TLS: {}, transaction journal: {}, audit log: {}, auth rate limit: {}, access log: {} — see ADR-0012/ADR-0014/ADR-0023/ADR-0025/ADR-0029/ADR-0030/ADR-0031/ADR-0036; do not expose beyond a trusted network unless auth and TLS are both configured)",

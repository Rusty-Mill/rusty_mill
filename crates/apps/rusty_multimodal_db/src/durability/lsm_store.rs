...
impl LsmStore {
    ...
    /// for why this splits into a spawned thread building the adjacency
    /// index (only touches `edges`) running alongside the breed index and
    /// records-map construction (only touches `records`) — same

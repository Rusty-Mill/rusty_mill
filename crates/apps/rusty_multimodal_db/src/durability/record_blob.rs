...
// The pieces every companion blob shares moved to the engine with the generic
// store (ADR-0124); this Dog blob keeps only its own magic, version and body.
use crate::record::DogRecord;
pub(crate) use rusty_multimodal_db_engine::durability::record_blob::{
    companion_path, encode_image, parse_header, EncodedRecordBlob, Fnv1a64, HEADER_LEN,
};
#[cfg(test)]
use rusty_multimodal_db_engine::durability::record_blob::{
    FINGERPRINT_OFFSET, TEMP_SUFFIX, VERSION_OFFSET,
};

...
fn rush_interactive(input: &str) -> (String, String) {
    ...
    let home =
        std::env::temp_dir().join(format!("rush_interactive_test_{}_{n}", std::process::id()));
...
fn read_builtin_shares_the_interactive_editors_input_stream() {
    ...
    assert!(
        out.contains("[hi]"),
        "read lost its line to the editor: {out:?}"
    );
}
...
fn read_builtin_from_a_redirected_file_is_scoped_to_the_call() {
    ...
    let (out, _) = rush(&format!(
        "read a < {file}; read b < {file}; echo \"[$a][$b]\""
    ));
...
fn errexit_matches_bashs_positionally_last_rule() {
    ...
    assert_eq!(rush("set -e; true && true && false; echo unreached").0, "");
...
fn nounset_rejects_unset_variable_references() {
    ...
    assert_eq!(
        rush("set -u; echo ${UNDEF:-default}; echo ok").0,
        "default\nok\n"
    );
    ...
    assert_eq!(
        rush("set -u; echo \"$*\"; echo \"$#\"; echo ok").0,
        "\n0\nok\n"
    );
...
fn xtrace_echoes_each_command_before_running_it() {
    ...
    assert_eq!(
        rush_stderr("set -x; echo a | tr a-z A-Z").0,
        "+ echo a\n+ tr a-z A-Z\n"
    );
    ...
    assert_eq!(
        rush_stderr("set -x; FOO=bar echo hi").0,
        "+ FOO=bar\n+ echo hi\n"
    );

    // `$PS4` is user-customizable.
    assert_eq!(
        rush_stderr("PS4='TRACE: '; set -x; echo hi").0,
        "TRACE: echo hi\n"
    );
...
fn term_and_hup_traps_fire_and_can_interrupt_a_blocking_wait() {
    ...
    assert_eq!(
        rush_signaled(
            r#"trap "echo caught; exit 5" TERM; sleep 0.6; echo unreached"#,
            libc::SIGTERM,
            100
        ),
        ("caught\n".to_string(), 5)
    ...
    assert_eq!(
        rush_signaled(
            r#"trap "echo caught" TERM; sleep 0.3; echo after"#,
            libc::SIGTERM,
            100
        ),
        ("caught\nafter\n".to_string(), 0)
    ...
    assert_eq!(
        rush_signaled(
            r#"trap "echo hup-caught; exit 7" HUP; sleep 0.6"#,
            libc::SIGHUP,
            100
        ),
        ("hup-caught\n".to_string(), 7)
...
fn indexed_arrays_basic_literal_index_and_whole_array_reads() {
    ...
    assert_eq!(
        rush(r#"arr=(a b c); echo "${arr[0]}" "${arr[2]}""#).0,
        "a c\n"
    );
    assert_eq!(
        rush(r#"arr=(a b c); echo "count=${#arr[@]}""#).0,
        "count=3\n"
    );
    // Out of range: empty, not an error.
    assert_eq!(
        rush(r#"arr=(a b c); echo "[${arr[10]}]"; echo status:$?"#).0,
        "[]\nstatus:0\n"
    );
...
fn indexed_arrays_are_sparse() {
    ...
    assert_eq!(
        rush(r#"arr=(a b); arr[5]=x; echo "${arr[@]}""#).0,
        "a b x\n"
    );
    ...
    assert_eq!(
        rush(r#"arr=(a b c); unset arr; echo "count=${#arr[@]}""#).0,
        "count=0\n"
    );
}
...
fn associative_arrays_declare_index_and_whole_array_reads() {
    ...
    // are `[key]=value` pairs.
    assert_eq!(
        rush(r#"declare -A arr=([a]=1 [b]=2); echo "${arr[a]}" "${arr[b]}""#).0,
        "1 2\n"
    );
    assert_eq!(
        rush(r#"declare -A arr=([a]=1 [b]=2); echo "count=${#arr[@]}""#).0,
        "count=2\n"
    );
    assert_eq!(
        rush(r#"declare -A arr=([a]=1 [b]=2); echo "${#arr[a]}""#).0,
        "1\n"
    );
...
fn associative_arrays_at_vs_star_and_key_iteration() {
    ...
    assert_eq!(
        rush(
            r#"declare -A arr=([a]=1 [b]=2); for k in "${!arr[@]}"; do echo "$k=${arr[$k]}"; done"#
        )
        .0
        .split('\n')
        .filter(|s| !s.is_empty())
...
fn associative_arrays_append_merge_and_unset_key() {
    ...
    assert_eq!(
        rush(r#"declare -A arr=([a]=1); arr[a]+=X; echo "${arr[a]}""#).0,
        "1X\n"
    );
...
fn local_assoc_array_scopes_to_the_function_call() {
    assert_eq!(
        rush(
            r#"f() { local -A arr=([a]=1 [b]=2); echo "${arr[a]}"; }; f; echo "outer:${#arr[@]}""#
        )
        .0,
        "1\nouter:0\n"
    );
...
fn nested_substitution_status_reflects_its_own_last_command() {
    ...
    assert_eq!(
        rush("x=$(y=$(false); echo inner); echo $x:$?").0,
        "inner:0\n"
    );
}
...
fn for_loop_without_in_clause_iterates_positional_params() {
    ...
    assert_eq!(
        rush("for x; do echo unreached; done; echo after").0,
        "after\n"
    );
    ...
    let (out, _) = rush_argv(
        "for x in; do echo unreached; done; echo after",
        &["dummy", "a", "b"],
    );
    assert_eq!(out, "after\n");
}
...
fn command_type_and_hash_builtins() {
    ...
    // fails.
    assert_eq!(
        rush("alias ll='ls -l'; command -v ll").0,
        "alias ll='ls -l'\n"
    );
    ...
    assert_eq!(
        rush("cd() { echo fake; }; cd; command cd; pwd")
            .0
            .lines()
            .next()
            .unwrap(),
...
fn wait_builtin_and_last_bg_pid() {
    ...
    assert_eq!(
        rush("(exit 5) & p=$!; wait $p; echo \"status=$?\"").0,
        "status=5\n"
    );

    // `wait %job` reports the job's (last-stage's) exit status.
    assert_eq!(
        rush("{ sleep 0.1; exit 7; } & wait %1; echo \"status=$?\"").0,
        "status=7\n"
    );
...
fn eval_builtin() {
    ...
    assert_eq!(
        rush("x=1; eval 'y=2; echo $x $y'; echo after:$x:$y").0,
        "1 2\nafter:1:2\n"
    );
    ...
    assert_eq!(
        rush("eval 'for i in a b c; do echo $i; done'").0,
        "a\nb\nc\n"
    );
...
fn source_and_dot_builtin() {
    ...
    assert_eq!(
        rush(&format!(". '{file}'; echo $FOO; greet world")).0,
        "bar\nhi world\n"
    );
    ...
    let args_path =
        std::env::temp_dir().join(format!("rush_source_args_{}.sh", std::process::id()));
    std::fs::write(&args_path, "echo \"args:$#:$*\"\n").unwrap();
    let args_file = args_path.to_str().unwrap();
    assert_eq!(
        rush(&format!(
            "f() {{ . '{args_file}'; echo \"after:$#:$*\"; }}; f a b c"
        ))
        .0,
        "args:3:a b c\nafter:3:a b c\n"
    );
    assert_eq!(
        rush(&format!(
            "f() {{ . '{args_file}' x y; echo \"after:$#:$*\"; }}; f a b c"
        ))
        .0,
        "args:2:x y\nafter:3:a b c\n"
    );
    ...
    let ret_path =
        std::env::temp_dir().join(format!("rush_source_return_{}.sh", std::process::id()));
    ...
    let brk_path =
        std::env::temp_dir().join(format!("rush_source_break_{}.sh", std::process::id()));
    std::fs::write(&brk_path, "echo in-lib\nbreak\necho unreached\n").unwrap();
    let brk_file = brk_path.to_str().unwrap();
    assert_eq!(
        rush(&format!(
            "for i in 1 2 3; do . '{brk_file}'; echo unreached-loop-body; done; echo after-loop"
        ))
        .0,
        "in-lib\nafter-loop\n"
    );
...
fn set_reassigns_positional_parameters() {
    ...
    assert_eq!(
        rush_argv("set -- newarg; echo $0", &["myprog"]).0,
        "myprog\n"
    );
...
fn compound_command_as_one_stage_of_a_real_pipeline() {
    ...
    assert_eq!(
        rush("if true; then echo yes; else echo no; fi | tr a-z A-Z").0,
        "YES\n"
    );
    ...
    assert_eq!(
        rush("echo hi | (cat; echo done) | tr a-z A-Z").0,
        "HI\nDONE\n"
    );
    ...
    assert_eq!(rush("cd /tmp && (cd /; pwd) | cat; pwd").0, "/\n/tmp\n");
}
...
fn read_builtin_splits_fields_and_reports_eof() {
    ...
    assert_eq!(
        rush_stdin("read; echo \"got:$REPLY\"", "hi\n").0,
        "got:hi\n"
    );
    ...
    assert_eq!(
        rush_stdin("read x y; echo \"[$x][$y]\"", "a b c d\n").0,
        "[a][b c d]\n"
    );
    // Fewer fields than names: the rest get the empty string.
    assert_eq!(
        rush_stdin("read x y z; echo \"[$x][$y][$z]\"", "a  b\n").0,
        "[a][b][]\n"
    );
    ...
    assert_eq!(
        rush_stdin("read x; echo \"$?:[$x]\"", "hello").0,
        "1:[hello]\n"
    );
...
fn read_backslash_escaping_and_raw_mode() {
    ...
    assert_eq!(
        rush_stdin("read x y; echo \"[$x][$y]\"", "a\\\nb c\n").0,
        "[ab][c]\n"
    );
    assert_eq!(
        rush_stdin("read x y; echo \"[$x][$y]\"", "a\\ b c\n").0,
        "[a b][c]\n"
    );

    // `-r` disables all of that: the backslash is just an ordinary character.
    assert_eq!(
        rush_stdin("read -r x y; echo \"[$x][$y]\"", "a\\ b c\n").0,
        "[a\\][b c]\n"
    );
}
...
fn redirect_trailing_a_compound_command_is_applied() {
    ...
    #[cfg(unix)]
    assert_eq!(
        rush(&format!("(cat) < {file} | tr a-z A-Z")).0,
        "ONE\nTWO\n"
    );
    // And when the whole compound is captured via `$(...)`.
    assert_eq!(
        rush(&format!("x=$(cat < {file}); echo \"$x\"")).0,
        "one\ntwo\n"
    );

    // Output redirect trailing a `while` loop.
    let out_path = std::env::temp_dir().join(format!(
        "rush_compound_redir_out_{}.txt",
        std::process::id()
    ));
    let out_file = shell_path(&out_path);
    rush(&format!(
        "i=0; while [ $i -lt 2 ]; do echo hi; i=$((i+1)); done > {out_file}"
    ));
...
fn case_semi_amp_falls_through_unconditionally() {
    ...
    assert_eq!(
        rush("case a in a) echo one;& b) echo two;; esac").0,
        "one\ntwo\n"
    );
    ...
    assert_eq!(
        rush("case a in a) false;& b) echo two;; esac; echo status:$?").0,
        "two\nstatus:0\n"
    );
}
...
fn case_dsemi_amp_resumes_pattern_testing() {
    ...
    assert_eq!(
        rush("case a in a) echo one;;& b) echo two;; a) echo three;; esac").0,
        "one\nthree\n"
    );
    ...
    assert_eq!(
        rush(r#"case a in a) echo one;;& esac; echo "after:$?""#).0,
        "one\nafter:0\n"
    );
}
...
fn select_reads_a_valid_index_or_blank_name_on_no_match() {
    ...
    assert_eq!(
        rush_stdin(
            "select x in a b c; do echo \"x=[$x] reply=[$REPLY]\"; break; done",
            "2\n"
        )
        .0,
        "x=[b] reply=[2]\n"
    ...
    assert_eq!(
        rush_stdin("select x in a b c; do echo \"x=$x\"; break; done", " 2 \n").0,
        "x=b\n"
    );
    assert_eq!(
        rush_stdin("select x in a b c; do echo \"x=$x\"; break; done", "+2\n").0,
...
fn select_blank_line_redisplays_without_running_the_body() {
    ...
    assert_eq!(
        rush_stdin(
            "select x in a b; do echo \"ran reply=[$REPLY]\"; break; done",
            "\n2\n"
        )
        .0,
        "ran reply=[2]\n"
...
fn select_ps3_prompt_defaults_and_dollar_expands() {
    ...
    assert_eq!(err, "1) a\n#? ");
    let (err, _) = rush_stdin_stderr(
        r#"name=world; PS3="pick $name> "; select x in a; do break; done"#,
        "1\n",
    );
    assert_eq!(err, "1) a\npick world> ");
}
...
fn c_style_for_loop_basic_and_all_clauses_optional() {
    ...
    assert_eq!(
        rush("i=0; for ((;i<3;)); do echo $i; i=$((i+1)); done").0,
        "0\n1\n2\n"
    );
}
...
fn standalone_arith_command_sets_exit_status_and_runs_side_effects() {
    ...
    assert_eq!(
        rush(r#"i=1; ((i = i + 5)); echo "$i status:$?""#).0,
        "6 status:0\n"
    );
...
fn arithmetic_assignment_and_inc_dec_in_dollar_paren() {
    ...
    assert_eq!(
        rush(r#"i=1; j=$((i = 10)); echo "i=$i j=$j""#).0,
        "i=10 j=10\n"
    );
...
fn arithmetic_short_circuit_skips_assignment_side_effects() {
    ...
    assert_eq!(
        rush("i=1; echo $((0 ? (i=9) : (i=7))); echo $i").0,
        "7\n7\n"
    );
}
...
fn arithmetic_bare_array_subscript_read_assign_and_inc_dec() {
    ...
    assert_eq!(
        rush(r#"a=(1 2 3); (( a[1] = 99 )); echo "${a[@]}""#).0,
        "1 99 3\n"
    );
    assert_eq!(
        rush(r#"a=(1 2 3); (( a[1]++ )); echo "${a[@]}""#).0,
        "1 3 3\n"
    );
    assert_eq!(
        rush(r#"a=(1 2 3); (( ++a[1] )); echo "${a[@]}""#).0,
    ...
    assert_eq!(
        rush(r#"a=(1 2 3); (( a[1] += 10 )); echo "${a[@]}""#).0,
        "1 12 3\n"
    );
    ...
    assert_eq!(
        rush("declare -A m=([x]=5 [y]=10); echo $((m[x] + 1))").0,
        "6\n"
    );
    assert_eq!(
        rush(r#"declare -A m=([x]=5 [y]=10); (( m[x] = 20 )); echo "${m[x]}""#).0,
        "20\n"
    );
...
fn brace_expansion_ranges() {
    ...
    assert_eq!(
        rush("for i in {1..3}; do echo n=$i; done").0,
        "n=1\nn=2\nn=3\n"
    );
}
...
fn brace_expansion_runs_before_dollar_expansion_and_skips_assignments() {
    ...
    assert_eq!(
        rush(r#"arr=({a,b} c); echo "${arr[@]}" "${#arr[@]}""#).0,
        "a b c 3\n"
    );
}
...
fn process_substitution_works_in_assignment_rhs_and_redirect_targets() {
    ...
    assert_eq!(
        rush(r#"x=$(cat <(echo inner)); echo "x=$x""#).0,
        "x=inner\n"
    );
    assert_eq!(
        rush(r#"read -r line < <(echo hello-read); echo "line=[$line]""#).0,
        "line=[hello-read]\n"
    );
}
...
fn local_attributes_are_function_scoped() {
    ...
    let (out, _) = rush(
        "v=Mixed; f(){ local -u v=abc; echo in=$v; v=ghi; echo in=$v; }; f; echo out=$v; v=next; echo $v",
    );
...
fn trap_rejects_invalid_specs_without_blocking_valid_ones() {
    ...
    let (out, _) = rush(
        "trap 'echo caught' TERM BOGUS 2>/dev/null; echo st=$?; kill -15 $$; sleep 0; echo after",
    );
    assert_eq!(out, "st=1\ncaught\nafter\n");

    let (err, status) = rush_stderr("trap 'echo x' 99");
    assert!(
        err.contains("99: invalid signal specification"),
        "got: {err:?}"
    );
    assert_eq!(status, 1);
}
...
fn readonly_listing_and_declare_r() {
    ...
    assert!(
        out.lines().any(|l| l == "declare -r x=\"1\""),
        "got: {out:?}"
    );
    let (out, _) = rush("readonly arr=(a b); readonly");
    assert!(
        out.lines()
            .any(|l| l == "declare -ar arr=([0]=\"a\" [1]=\"b\")"),
        "got: {out:?}"
    );
...
fn ulimit_reads_and_sets_limits() {
    ...
    let (out, _) = rush(
        "hard=$(ulimit -H -n); ulimit -S -n 512; echo \"$(ulimit -n) $([ \"$(ulimit -H -n)\" = \"$hard\" ] && echo same)\"",
    );
    ...
    assert!(
        out.lines()
            .any(|l| l.starts_with("open files") && l.contains("-n")),
        "got: {out:?}"
    );
...
fn command_p_uses_the_default_system_path() {
    ...
    assert!(
        err.contains("totallynonexistent_c47: command not found"),
        "got: {err:?}"
    );
    assert!(
        !err.contains("totallynonexistent_c47/"),
        "synthetic slash leaked: {err:?}"
    );
}
...
fn noclobber_refuses_overwrite_and_clobber_overrides() {
    ...
    assert_eq!(out, "st=0\ny\n");
    let (out, _) = rush(&format!(
        "set -C; echo a >> {f}; echo z > /dev/null; echo st=$?"
    ));
    assert_eq!(out, "st=0\n");
    ...
    // gains/loses C.
    let (out, _) = rush(&format!(
        "set -C; set +C; echo ok > {f}; cat {f}; echo n >| {f}; cat {f}"
    ));
    assert_eq!(out, "ok\nn\n");
...
fn set_o_long_names_and_listing() {
    ...
    assert_eq!((out.as_str(), status), ("", 1));
    let (out, _) =
        rush("set -o nounset; echo ${UNSET_C52-fallback}; set +o nounset; echo ok$UNSET_C52");
    ...
    assert!(
        out.lines().any(|l| l == "errexit        \ton"),
        "got: {out:?}"
    );
    assert!(
        out.lines().any(|l| l == "pipefail       \toff"),
        "got: {out:?}"
    );
    ...
    let (out, _) =
        rush("set -o noclobber; saved=$(set +o); set +o noclobber; eval \"$saved\"; echo [$-]");
    assert_eq!(out, "[hBCc]\n");
...
fn trap_err_fires_on_failing_commands() {
    ...
    let (out, _) = rush(
        "trap 'echo E' ERR; if false; then :; fi; false && echo x; ! true; f(){ false; true; }; f; echo end",
    );
    assert_eq!(out, "end\n");
    ...
    let (out, _) =
        rush("trap 'echo E' ERR; true && false; true | false; f(){ return 3; }; f; echo end");
    assert_eq!(out, "E\nE\nE\nend\n");
...
fn pipestatus_records_per_stage_statuses() {
    ...
    let (out, _) = rush(
        r#"sh -c 'exit 3' | sh -c 'exit 5' | true; echo "${PIPESTATUS[@]} ${PIPESTATUS[1]} ${#PIPESTATUS[@]}""#,
    );
    ...
    assert_eq!(out, "1\n1\n");
    let (out, _) =
        rush(r#"if true; then :; fi; echo "${PIPESTATUS[@]}"; x=5; echo "${PIPESTATUS[@]}""#);
    assert_eq!(out, "0\n0\n");
...
fn double_bracket_extended_test() {
    ...
    let (out, _) =
        rush(r#"x=; [[ $x = foo ]]; echo st=$?; x="a b"; [[ $x = "a b" ]] && echo split-safe"#);
    assert_eq!(out, "st=1\nsplit-safe\n");

    // Unquoted RHS is a glob pattern (even via $var); quoted is literal.
    let (out, _) = rush(
        r#"x=foo.txt; [[ $x = *.txt ]] && echo glob; [[ $x = "*.txt" ]] || echo lit; p="*.txt"; [[ $x = $p ]] && echo varpat; [[ abc = "a"* ]] && echo mixed"#,
    );
    assert_eq!(out, "glob\nlit\nvarpat\nmixed\n");

    // `<`/`>` compare lexicographically — no redirection, no file.
    let (out, _) =
        rush("[[ abc < abd ]] && echo lt; [[ abd > abc ]] && echo gt; [ -e abd ] || echo nofile");
    assert_eq!(out, "lt\ngt\nnofile\n");

    // &&/||/!/( ) nest directly; -eq family is full arithmetic.
    let (out, _) = rush(
        "[[ ( a = b || a = a ) && c = c ]] && echo grouped; [[ ! -f nosuch_c55 ]] && echo notfile; x=5; [[ x -eq 5 ]] && echo arith",
    );
    assert_eq!(out, "grouped\nnotfile\narith\n");

    // Unary file/string ops, POSIX classes inside [[, and `if [[ … ]]`.
    let (out, _) = rush(
        "[[ -f Cargo.toml && -d src && -n x && -z \"\" ]] && echo ops; [[ 5 = [[:digit:]] ]] && echo class; if [[ -d src ]]; then echo in-if; fi",
    );
    assert_eq!(out, "ops\nclass\nin-if\n");
...
fn regex_match_and_bash_rematch() {
    ...
    let (out, _) = rush(
        r#"[[ "abc123" =~ ([a-z]+)([0-9]+) ]] && echo "${BASH_REMATCH[0]}|${BASH_REMATCH[1]}|${BASH_REMATCH[2]}""#,
    );
    assert_eq!(out, "abc123|abc|123\n");

    // Quantifiers, anchors, and the $var idiom; quoted RHS is literal.
    let (out, _) = rush(
        r#"[[ 2024-01-15 =~ ^([0-9]{4})-([0-9]{2}) ]] && echo "y=${BASH_REMATCH[1]} m=${BASH_REMATCH[2]}"; p="^a.c$"; [[ abc =~ $p ]] && echo var; [[ abc =~ "a.c" ]] || echo quoted-literal"#,
    );
    ...
    let (out, _) = rush(
        r#"[[ abc =~ (x)?(b) ]] && echo "n=${#BASH_REMATCH[@]} [${BASH_REMATCH[1]}] [${BASH_REMATCH[2]}]"; [[ abc =~ z ]]; echo "st=$? [${BASH_REMATCH[0]}]""#,
    );
    assert_eq!(out, "n=3 [] [b]\nst=1 []\n");
    ...
    let (out, _) = rush(
        r#"[[ "a b" =~ (a b) ]] && echo group; [[ a.c =~ a\.c ]] && echo esc; [[ abc =~ a\.c ]] || echo esc2; [[ abc =~ a(b)c && x = x ]] && echo "combined ${BASH_REMATCH[1]}""#,
    );
    assert_eq!(out, "group\nesc\nesc2\ncombined b\n");
    ...
    let (out, _) = rush(
        r#"[[ abc =~ "(" ]]; echo st=$?; p='['; [[ abc =~ $p ]] 2>/dev/null; echo st=$?; echo alive"#,
    );
    assert_eq!(out, "st=1\nst=2\nalive\n");
}
...
fn extended_globs_across_surfaces() {
    ...
    let (out, _) = rush(
        "for s in afile bfile cfile abfile; do case $s in @(a|b)file) echo \"$s: at\";; *) :;; esac; done",
    );
    assert_eq!(out, "afile: at\nbfile: at\n");

    // [[ ]] pattern matching and the ${v%%pat} family share the matcher.
    let (out, _) = rush(
        "[[ aaa = +(a) ]] && echo plus; [[ cfile = !(a|b)file ]] && echo neg; v=foo.tar.gz; echo ${v%%+(.*)}",
    );
    assert_eq!(out, "plus\nneg\nfoo\n");
    ...
    let cd = format!("cd {}; ", dir.display());
    let (out, _) = rush(&format!(
        "{cd}echo @(a|b)file; echo !(a|b)file; echo +(a|b)file"
    ));
...
fn shopt_and_glob_options() {
    ...
    let (out, _) = rush(
        "shopt nullglob; echo st=$?; shopt -q nullglob; echo st=$?; shopt -s nullglob; shopt -q nullglob; echo st=$?",
    );
    ...
    let (out, _) = rush(
        "shopt -u extglob; [[ afile = @(a|b)file ]]; echo st=$?; shopt -s extglob; [[ afile = @(a|b)file ]] && echo back-on",
    );
    assert_eq!(out, "st=1\nback-on\n");
...
fn string_transformation_operators() {
    ...
    // bash byte-for-byte.
    let (out, _) = rush(
        r#"v=hello_world; echo "${v/o/0}" "${v//o/0}" "${v/#hello/HI}" "${v/%world/EARTH}" "${v/o}" "${v//l}""#,
    );
    assert_eq!(
        out,
        "hell0_world hell0_w0rld HI_world hello_EARTH hell_world heo_word\n"
    );

    // Substrings: negative offset (space-disambiguated), negative
    // length, arithmetic expressions, out-of-range → empty.
    let (out, _) = rush(
        r#"v=abcdef; echo "${v:2}" "${v:2:3}" "${v: -3}" "${v: -4:2}" "${v:1:-2}"; n=2; echo "${v:n+1:2}"; echo "[${v:9}]" "[${v: -10}]""#,
    );
    assert_eq!(out, "cdef cde def cd bcd\nde\n[] []\n");

    // Case conversion, with and without a pattern restriction.
    let (out, _) =
        rush(r#"v=hello; echo "${v^}" "${v^^}" "${v^^[a-f]}"; V=HELLO; echo "${V,}" "${V,,}""#);
    assert_eq!(out, "Hello HELLO hEllo\nhELLO hello\n");

    // Glob patterns in search/replace; escaped `/` as the pattern.
    let (out, _) = rush(
        r#"v=aXbXc; echo "${v/X*/Z}" "${v//X/-}" "${v/[Xb]/_}"; p=a/b/c; echo "${p/\//_}" "${p//\//.}""#,
    );
...
fn array_wide_transformations_and_slicing() {
    ...
    let (out, _) =
        rush(r#"arr=(one two); echo "${arr[@]/o/0}"; echo "${arr[@]^^}"; echo "${arr[@]#o}""#);
    assert_eq!(out, "0ne tw0\nONE TWO\nne two\n");

    let (out, _) =
        rush(r#"arr=(a b c d); echo "${arr[@]:1:2}"; echo "${arr[@]: -1}"; echo "${arr[@]:2}""#);
    assert_eq!(out, "b c\nd\nc d\n");
}
...
fn indirect_expansion_and_name_listing() {
    ...
    let (out, _) =
        rush(r#"x=hello; v=x; echo "${!v}"; set -- one two; n=2; echo "${!n}"; echo "${!v:-def}""#);
    assert_eq!(out, "hello\ntwo\nhello\n");
...
fn at_transformations() {
    ...
    let (out, _) = rush(
        r#"declare -ir n=5; echo "${n@a}"; arr=(a b); echo "${arr@a}"; declare -A m; echo "${m@a}"; x=plain; echo "[${x@a}]""#,
    );
    assert_eq!(out, "ir\na\nA\n[]\n");
...
fn mapfile_reads_lines_into_an_array() {
    ...
    let (out, _) = rush(&format!(
        "mapfile -t lines < {path}; echo \"${{lines[@]}} n=${{#lines[@]}}\"; echo \"${{lines[1]}}\""
    ));
    ...
    let (out, _) = rush(&format!(
        "mapfile raw < {path}; printf '[%s]' \"${{raw[0]}}\"; echo"
    ));
    assert_eq!(out, "[l1\n]\n");
...
fn nameref_variables() {
    ...
    let (out, _) =
        rush(r#"declare -n ref=arr; arr=(a b); echo "${ref[1]}"; ref[0]=Z; echo "${arr[0]}""#);
    assert_eq!(out, "b\nZ\n");
    ...
    let (out, _) =
        rush(r#"f(){ local -n out=$1; out=(a b c); }; f myarr; echo "${myarr[1]} ${#myarr[@]}""#);
    assert_eq!(out, "b 3\n");
...
fn job_control_niceties() {
    ...
    assert!(
        out.contains("15) SIGTERM") && out.contains("10) SIGUSR1"),
        "got: {out:?}"
    );

    // jobs -p prints just the pgid; -l includes it in the long line.
    let (out, _) = rush(
        "sleep 2 & p=$(jobs -p); [ \"$p\" -gt 0 ] && echo p-ok; l=$(jobs -l); case $l in \"[1]  $p Running\"*) echo l-ok;; esac; kill %1",
    );
    assert_eq!(out, "p-ok\nl-ok\n");
    ...
    let (out, _) = rush(
        "sleep 2 & pid=$!; disown; jobs; kill %1 2>/dev/null; echo kill-st=$?; kill $pid; echo done",
    );
...
fn coproc_bidirectional_pipes() {
    ...
    let (out, _) = rush(
        r#"coproc cat; echo hello >&"${COPROC[1]}"; read line <&"${COPROC[0]}"; echo "got: $line"; kill $COPROC_PID 2>/dev/null; echo done"#,
    );
    assert_eq!(out, "got: hello\ndone\n");

    // The named form takes a `{ ... }` group; NAME[0]/NAME[1]/NAME_PID.
    let (out, _) = rush(
        r#"coproc up { cat; }; echo hi >&"${up[1]}"; read x <&"${up[0]}"; echo "got=$x"; kill $up_PID 2>/dev/null"#,
    );
    ...
    // same as bash.
    let (out, _) = rush(
        r#"coproc cat; [ "$COPROC_PID" = "$!" ] && [ "$COPROC_PID" -gt 0 ] && echo pids-ok; kill $COPROC_PID; wait $COPROC_PID 2>/dev/null; echo waited-st=$?"#,
    );
...
fn special_variables_grab_bag() {
    ...
    let (out, _) = rush(
        r#"a=$RANDOM; [ "$a" -ge 0 ] && [ "$a" -le 32767 ] && echo range-ok; RANDOM=42; a=$RANDOM; RANDOM=42; b=$RANDOM; [ "$a" = "$b" ] && echo seeded"#,
    );
    assert_eq!(out, "range-ok\nseeded\n");

    let (out, _) = rush(
        r#"SECONDS=100; echo $SECONDS; [ "${EPOCHREALTIME%.*}" = "$EPOCHSECONDS" ] && echo epoch-ok"#,
    );
    assert_eq!(out, "100\nepoch-ok\n");
...
fn abbr_builtin_manages_the_table() {
    ...
    let (out, _) =
        rush("abbr gs='git status'; abbr; abbr gs; unabbr gs; abbr gs 2>/dev/null; echo st=$?");
...
fn cd_niceties() {
    ...
    let (out, _) = rush(
        "cd /tmp; pushd /usr > /dev/null; pushd /etc >/dev/null; dirs; popd >/dev/null; dirs; pushd >/dev/null; dirs",
    );
...
fn negative_array_subscripts() {
    ...
    let (out, _) = rush(
        r#"a=(x y z); echo "${a[-1]}" "${a[-3]}"; a[-1]=Q; echo "${a[@]}"; unset "a[-1]"; echo "${a[@]}""#,
    );
    assert_eq!(out, "z x\nx y Q\nx y\n");
...
fn positional_parameter_slicing_and_count() {
    ...
    // bash.
    let (out, code) = rush_argv(
        r#"echo "${@:2:2}"; echo "${@:3}"; echo "${#*} ${#@}"; echo "${@: -1}"; echo "${@:0:2}""#,
        &["zero", "a", "b", "c", "d"],
    );
    assert_eq!(out, "b c\nc d\n4 4\nd\nzero a\n");
    assert_eq!(code, 0);
...
fn at_transform_case_key_and_prompt_forms() {
    ...
    let (out, _) = rush(
        r#"a=(one two); echo "${a[@]@U}"; declare -A m=([x]=1); echo "${m[@]@k}"; echo "${m[@]@K}""#,
    );
...
fn at_k_uses_bashs_own_double_quoted_style_not_at_qs() {
    ...
    assert_eq!(
        out,
        r#"0 "a b" 1 "c\$d" 2 "e\"f" 3 "g\\h" 4 "i\`j"
"#
    );
...
fn whole_array_at_nested_inside_command_substitution_inside_double_quotes() {
    ...
    let (out, _) = rush(r#"a=(1 2 3); for i in "$(printf "%s-" "${a[@]}")"; do echo "$i"; done"#);
    assert_eq!(out, "1-2-3-\n");
...
fn arithmetic_literals_and_string_reeval() {
    ...
    let (out, _) =
        rush("echo $((2#101)) $((16#ff)) $((8#17)) $((36#z)) $((64#_@)) $((0xff)) $((010))");
...
fn let_builtin() {
    ...
    // variables.
    let (out, _) = rush(
        r#"let x=3+4 y=x*2; echo $x $y; let i=5; let i++; echo $i; n=1; let "n = n + 41"; echo $n"#,
    );
    assert_eq!(out, "7 14\n6\n42\n");
...
fn echo_escape_flags() {
    ...
    let (out, _) =
        rush(r#"echo -e "a\tb"; echo -en "x\ty"; echo; echo -E "a\tb"; echo -e "one\ctwo"; echo"#);
    assert_eq!(out, "a\tb\nx\ty\na\\tb\none\n");
...
fn builtin_builtin_and_unset_f() {
    ...
    let (out, _) = rush(
        "f(){ :; }; unset -f f; type f 2>/dev/null; echo st=$?; g(){ :; }; unset g; type g 2>/dev/null; echo st=$?",
    );
    assert_eq!(out, "st=1\nst=1\n");
}
...
fn test_v_o_and_string_comparison() {
    ...
    let (out, _) =
        rush(r#"x=1; test -v x; echo $?; test -v nosuch; echo $?; a=(q); test -v "a[0]"; echo $?"#);
    assert_eq!(out, "0\n1\n0\n");
...
fn assorted_flag_batch() {
    ...
    let (out, _) = rush(
        "cd /; pushd /tmp >/dev/null; pushd /usr >/dev/null; dirs -v; popd +1 >/dev/null; dirs",
    );
    assert_eq!(out, " 0  /usr\n 1  /tmp\n 2  /\n/usr /\n");
}
...
fn declare_p_and_function_introspection() {
    ...
    let (out, _) = rush(
        "f(){ :; }; g(){ :; }; declare -F; declare -F f; echo st=$?; declare -F nosuch; echo st=$?; declare -f f >/dev/null; echo st=$?; declare -f nosuch >/dev/null; echo st=$?",
    );
    assert_eq!(
        out,
        "declare -f f\ndeclare -f g\nf\nst=0\nst=1\nst=0\nst=1\n"
...
fn export_n_unexports() {
    ...
    let (out, _) =
        rush(r#"export FOO=bar; export -n FOO; sh -c 'echo "x${FOO}x"'; echo "still=$FOO""#);
    assert_eq!(out, "xx\nstill=bar\n");
}
...
fn time_reserved_word() {
    ...
    assert!(
        err.contains("real\t0m") && err.contains("user\t0m") && err.contains("sys\t0m"),
        "got: {err:?}"
    );
    assert_eq!(code, 0);
...
fn standard_variables_seeded() {
    ...
    let (out, _) = rush(
        "echo ${UID:+u} ${EUID:+e} ${OSTYPE:+o} ${HOSTTYPE:+t} ${MACHTYPE:+m} ${RUSH_VERSION:+v}",
    );
...
fn bash_version_compat_vars() {
    ...
    let (out, _) = rush(
        r#"echo "$BASH_VERSION"; echo "${BASH_VERSINFO[0]}.${BASH_VERSINFO[1]}.${BASH_VERSINFO[2]}""#,
    );
...
fn dirstack_variable_tracks_pushd_popd_and_cd() {
    ...
    assert!(
        out.contains(&format!("STACK1={b_display} {a_display}\n")),
        "got: {out:?}"
    );
    assert!(
        out.contains(&format!("STACK2={a_display}\n")),
        "got: {out:?}"
    );
...
fn shopt_xpg_echo_and_nocasematch() {
    ...
    assert_eq!(out, "a\tb\n");
    let (out, _) = rush(
        "shopt -s nocasematch; [[ ABC == abc ]] && echo yes; case FOO in foo) echo case-yes;; esac",
    );
    assert_eq!(out, "yes\ncase-yes\n");
...
fn prefix_assignments_scope_to_builtins() {
    ...
    let (out, _) = rush(
        r#"FOO=1 true; echo "${FOO:-none}"; f(){ echo "in=$FOO"; }; FOO=baz f; echo "after=${FOO:-none}""#,
    );
    assert_eq!(out, "none\nin=baz\nafter=none\n");
}
...
fn quoted_array_expansion_with_adjacent_text() {
    ...
    assert_eq!(out, "[x1][2y]\n");
    let (out, _) =
        rush(r#"set -- p q r; printf "[%s]" "x$@y"; echo; set --; printf "[%s]" "x$@y"; echo"#);
    assert_eq!(out, "[xp][q][ry]\n[xy]\n");
    let (out, _) = rush(
        r#"a=(only); printf "[%s]" "x${a[@]}y"; echo; a=(1 2); printf "[%s]" "L${!a[@]}R"; echo"#,
    );
...
fn history_builtin_and_hist_variables() {
    ...
    assert!(
        out.contains("1  for i in 1 2; do; echo $i; done\n"),
        "got: {out:?}"
    );

    // HISTCONTROL=ignorespace (C122): the leading-space privacy idiom.
    std::fs::remove_file(home.join(".rush_history")).unwrap();
    let out = rush_session(
        &home,
...
fn declare_f_prints_and_roundtrips_function_source() {
    ...
    let (out, _) = rush(
        r#"f(){ echo hi; [[ x == x ]] && echo cond-ok; for i in a b; do echo $i; done; }; src=$(declare -f f); unset -f f; eval "$src"; f"#,
    );
    assert_eq!(out, "hi\ncond-ok\na\nb\n");
...
fn export_f_crosses_into_children() {
    ...
    let output = Command::new("bash")
        .arg("-c")
        .arg(format!(
            "f(){{ echo bash-to-rush; }}; export -f f; {} -c f",
            env!("CARGO_BIN_EXE_rush")
        ))
        .output()
        .expect("spawn bash");
...
fn small_wins_batch() {
    ...
    let (out, _) =
        rush(r#"declare -A a; a["x y"]=1; echo "${a[x y]}"; k="p q"; a["$k"]=2; echo "${a[$k]}""#);
    assert_eq!(out, "1\n2\n");

    // SHELLOPTS/BASHOPTS reflect live option state.
    let (out, _) = rush(
        r#"echo "[$SHELLOPTS]"; set -e; echo $SHELLOPTS | grep -qc errexit && echo reflected"#,
    );
    assert_eq!(
        out,
    ...
    let (out, _) =
        rush("shopt -s dotglob; echo $BASHOPTS | grep -qc dotglob && echo bash-reflected");
    ...
    assert_eq!(out, "2\n");
    let (out, _) =
        rush("help nosuchxyz 2>/dev/null; echo st=$?; caller; echo st=$?; f(){ caller; }; f");
    assert_eq!(out, "st=1\nst=1\n1 NULL\n");
...
fn varfd_named_descriptors() {
    ...
    let (out, _) = rush(
        r#"exec {x}>/dev/null; [ "$x" -ge 10 ] && echo alloc-ok; echo hi >&$x; exec {y}>&1; echo "via-y" >&$y"#,
    );
...
fn fc_builtin() {
    ...
    let (out, _) = rush(
        r#"history -s "echo a"; history -s "echo b"; history -s "echo c"; fc -l 2 3; fc -lr 2 3; fc -ln 3 3; fc -l -2 -1"#,
    );
    assert_eq!(
        out,
        "2\t echo b\n3\t echo c\n3\t echo c\n2\t echo b\n\t echo c\n2\t echo b\n3\t echo c\n"
...
fn invocation_flags() {
    ...
    let run = |args: &[&str]| {
        let o = Command::new(env!("CARGO_BIN_EXE_rush"))
            .args(args)
            .output()
            .unwrap();
        (
    ...
    let o = Command::new(env!("CARGO_BIN_EXE_rush"))
        .args(["-O", "nullglob", "-c", "shopt nullglob"])
        .output()
        .unwrap();
    assert!(String::from_utf8_lossy(&o.stdout).contains("on"));
    let o = Command::new(env!("CARGO_BIN_EXE_rush"))
        .args(["-lc", "shopt login_shell"])
        .output()
        .unwrap();
    assert!(String::from_utf8_lossy(&o.stdout).contains("on"));
...
fn nocaseglob_and_shopt_behaviors() {
    ...
    let (out, _) = rush(&format!(
        "cd {}; shopt -s nocaseglob; echo *.txt",
        dir.display()
    ));
...
fn programmable_completion() {
    ...
    let (out, _) =
        rush("complete -W x foo; complete | grep -c foo; complete -r foo; complete | grep -c foo");
    assert_eq!(out, "1\n0\n");
...
fn histtimeformat_and_native_read_s() {
    ...
    assert!(
        file.lines()
            .any(|l| l.starts_with('#') && l[1..].chars().all(|c| c.is_ascii_digit())),
...
fn bind_builtin_registers_without_error() {
    ...
    let (out, code) = rush(
        r#"bind "\C-t: forward-word"; bind -x "\C-g: echo hi"; bind "set completion-ignore-case on"; echo ok"#,
    );
    assert_eq!(out, "ok\n");
    assert_eq!(code, 0);
...
fn array_element_with_operator() {
    ...
    let (out, _) =
        rush(r#"a=(x y); echo "${a[0]:-none}|${a[5]:-def}|${a[0]#x}|${a[1]/y/Y}|${a[0]^^}""#);
...
fn bashpid_and_dollar_stability() {
    ...
    let (out, _) = rush(
        r#"echo "$(( BASHPID > 0 ))"; ( echo "$(( $$ == PARENT ))" ) 2>/dev/null; outer=$$; ( [ "$$" = "$outer" ] && echo same; [ "$BASHPID" != "$outer" ] && echo differs )"#,
    );
    assert!(
...
fn declare_redeclare_preserves_and_setu_arrays() {
    ...
    // element access.
    let (out, _) = rush(
        "declare -a x=(a b); declare -a x; echo \"[${x[@]}]\"; declare -A m=([k]=v); declare -A m; echo \"[${m[k]}]\"",
    );
    assert_eq!(out, "[a b]\n[v]\n");
...
fn test_builtin_file_operators_and_grouping() {
    ...
    let (out, _) = rush(
        r#"[ -t 1 ]; echo notty=$?; [ \( a = a \) ]; echo $?; [ \( -n x \) -a \( -z "" \) ]; echo $?"#,
    );
...
fn expansion_transforms_and_export_array() {
    ...
    let (out, _) = rush(
        r#"a=(x y); echo "${a[@]:-DEF}"; b=(); echo "${b[@]:-DEF}"; echo "${a[@]:+SET}"; echo "${b[@]:+SET}""#,
    );
    assert_eq!(out, "x y\nDEF\nSET\n\n");
...
fn mapfile_skip_count_origin_callback() {
    ...
    assert_eq!(
        rush_stdin(r#"mapfile -t -s 1 arr; echo "${arr[@]}""#, "a\nb\nc\nd\n").0,
        "b c d\n"
    );
    assert_eq!(
        rush_stdin(r#"mapfile -t -n 2 arr; echo "${arr[@]}""#, "a\nb\nc\nd\n").0,
        "a b\n"
    );
    assert_eq!(
        rush_stdin(
            r#"arr=(x y z w q r); mapfile -t -O 2 arr; echo "${arr[@]}""#,
            "a\nb\n"
        )
        .0,
        "x y a b q r\n"
    ...
    assert_eq!(
        rush_stdin(r#"mapfile -t -c 1 -C "echo cb" arr"#, "a\nb\n").0,
        "cb 0 a\ncb 1 b\n"
    );
    assert_eq!(
        rush_stdin(r#"mapfile -c 2 -C "echo CB" -t arr"#, "a\nb\nc\nd\ne\n").0,
        "CB 1 b\nCB 3 d\n"
    );
}
...
fn parameter_error_carries_name_prefix() {
    ...
    assert!(
        rush_stderr("unset x; echo ${x:?is empty}")
            .0
            .contains("x: is empty")
    );
    assert!(
        rush_stderr("unset foo; echo ${foo?}")
            .0
            .contains("foo: parameter not set")
    );
    assert!(
        rush_stderr("foo=; echo ${foo:?}")
            .0
            .contains("foo: parameter null or not set")
    );
    assert!(
        rush_stderr("a=(); echo ${a[0]:?boom}")
            .0
            .contains("a[0]: boom")
    );
}
...
fn return_trap_inheritance_follows_functrace() {
    ...
    assert_eq!(
        rush("f(){ trap 'echo R' RETURN; echo in-f; }; f; echo after").0,
        "in-f\nR\nafter\n"
    );
    assert_eq!(
        rush("trap 'echo R' RETURN; f(){ echo in-f; }; f; echo after").0,
        "in-f\nafter\n"
    );
    assert_eq!(
        rush("set -T; trap 'echo R' RETURN; f(){ echo in-f; }; f; echo after").0,
        "in-f\nR\nafter\n"
    );
    ...
    assert_eq!(
        rush("trap 'echo e' ERR; trap -p ERR").0,
        "trap -- 'echo e' ERR\n"
    );
}
...
fn compgen_double_dash_ends_options() {
    ...
    assert_eq!(
        rush("compgen -W \"apple apricot banana\" -- ap").0,
        "apple\napricot\n"
    );
    assert_eq!(
        rush("compgen -W \"one two three\" -- \"\"").0,
        "one\ntwo\nthree\n"
    );
}
...
fn backtick_command_substitution() {
    ...
    assert_eq!(
        rush("for i in `seq 1 3`; do echo -n $i; done; echo").0,
        "123\n"
    );
...
fn read_poll_and_delimiter_trimming() {
    ...
    assert_eq!(
        rush("read -t 0 x </dev/null; echo \"[${x-unset}]\"").0,
        "[unset]\n"
    );
    ...
    assert_eq!(
        rush_stdin(r#"read -d "" x; echo "[$x]""#, "l1\nl2\n").0,
        "[l1\nl2]\n"
    );
    assert_eq!(
        rush_stdin(r#"read -d "" x; echo "${#x}""#, "a\nb\n\n").0,
        "3\n"
    );
...
fn ulimit_real_time_row_present() {
    ...
    let (out, _) = rush("ulimit -a");
    assert!(
        out.lines().next().unwrap().contains("(microseconds, -R)"),
        "got: {out:?}"
    );
    let (r, code) = rush("ulimit -R");
    assert_eq!(code, 0);
    assert!(
        r.trim() == "unlimited" || r.trim().parse::<u64>().is_ok(),
        "got: {r:?}"
    );
}
...
fn parity_audit_edge_cases() {
    ...
    assert_eq!(
        rush("unset v; echo [${v-default}] [${v:-default}]").0,
        "[default] [default]\n"
    );
    assert_eq!(
        rush("v=''; echo [${v-default}] [${v:-default}]").0,
        "[] [default]\n"
    );
    ...
    assert_eq!(
        rush("v='foo/bar/baz.txt'; echo ${v%.txt}").0,
        "foo/bar/baz\n"
    );
    ...
    assert_eq!(
        rush("v='apple banana apple'; echo ${v/apple/orange}").0,
        "orange banana apple\n"
    );
    assert_eq!(
        rush("v='apple banana apple'; echo ${v//apple/orange}").0,
    ...
    assert_eq!(
        rush("v='Hello World'; echo ${v^^} ${v,,}").0,
        "HELLO WORLD hello world\n"
    );

    // 6. Variable indirection (${!v})
    assert_eq!(
        rush("target='secret'; ptr='target'; echo ${!ptr}").0,
        "secret\n"
    );
    ...
    assert_eq!(
        rush("v='12345'; [[ $v =~ ^[0-9]+$ ]] && echo numeric").0,
        "numeric\n"
    );

    // 8. Positional parameter forwarding ("$@")
    assert_eq!(
        rush(r#"set -- a "b c" d; for x in "$@"; do echo "[$x]"; done"#).0,
        "[a]\n[b c]\n[d]\n"
    );

    // 9. Associative array merge (declare -A)
    assert_eq!(
        rush("declare -A map; map+=([k1]=v1 [k2]=v2); echo ${map[k1]} ${map[k2]}").0,
        "v1 v2\n"
    );

    // 10. Case fallthrough (;& and ;;&)
    assert_eq!(
        rush("case 'a' in a) echo 1 ;& b) echo 2 ;; esac").0,
        "1\n2\n"
    );
    assert_eq!(
        rush("case 'a' in a) echo 1 ;;& a) echo 2 ;; esac").0,
        "1\n2\n"
    );

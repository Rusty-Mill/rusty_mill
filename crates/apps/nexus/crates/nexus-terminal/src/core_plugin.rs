...
use crate::activity::build_activity_entry;
use crate::adhoc::SqliteAdHocStore;
// R8 / #191 — wire types now live in `crate::ipc`; pull in the ones the
// non-test paths still use by bare identifier (the `publish_chunk`
// payload, the per-plugin `ReplInfo` registry) so the call sites that
...
// ── DTOs ───────────────────────────────────────────────────────────────────
// R8 / #191 — lifted into `crate::ipc` so this module stays focused on

...
- **Editing & content:** `editor`, `outline`, `paneMode`, `comments`, `crdtConflict`, `templates`, `linkSuggest`, `dailyNotes` (idempotent open-today, prev/next navigation, month-calendar pane — C6 #359), `uniqueNote` (Zettelkasten timestamp-id notes — RFC 0009), `randomNote` (open a random note — RFC 0009), `noteComposer` (merge into / extract selection / create from title — RFC 0009), `fileProperties` (typed frontmatter editor with per-key type picker — RFC 0009), `propertiesView` (all-notes properties table — RFC 0009)

...
- **A pull-based OTLP exporter** (`rk_observe::OtlpExporter`) — the observability wiring, bound to the same `KernelEvent` stream and carrying token/latency attributes + tool-status counters ([`RUSTYKEYS_OTLP_ENDPOINT`](../reference/configuration.md); inert when unset; cost stays 0 until a pricing table lands). **Pull-based** — accumulated at the host boundary and scraped via the gateway `GET /metrics` route — so a `sandboxed` isolation profile ([ADR-0030](../adr/0030-capability-isolation-toolexecutor.md)) cannot blind operators.

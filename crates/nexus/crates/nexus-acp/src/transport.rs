...
mod tests {
    ...
    async fn caps_unterminated_line_before_it_grows_unboundedly() {
        ...
        /// stands in for an agent sending an unterminated multi-
        /// gigabyte line. If `read_message` only checked the line
        /// length *after* `read_line` returned, this would drive
        /// unbounded buffer growth and never terminate; the
        /// preventive `.take(MAX_LINE_BYTES)` wrapper must stop the
        /// read at the cap regardless.
        struct InfiniteJunk;

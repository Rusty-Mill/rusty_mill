...
// Algolia's wire protocol goes through reqwest's real-serde-backed
// `.json()` convenience methods, so `serde_json::Value` stays this
// crate's HTTP-body type throughout `lib.rs`/`query_map.rs`.

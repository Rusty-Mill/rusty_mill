...
/// Publish a `com.nexus.storage.*` state-change event, logging at warn
/// if the bus rejects it. Audit gap A2: subscribers (shell file tree,
/// FTS reconciler, etc.) silently desync when a publish drops.
fn publish_storage_event(bus: &EventBus, type_id: &str, payload: serde_json::Value) {
    if let Err(err) = bus.publish_plugin(PLUGIN_ID, type_id, payload) {
        tracing::warn!(
            plugin_id = PLUGIN_ID,
            event_type = type_id,
            %err,
            "storage event dropped — bus publish failed",
        );
    }
}

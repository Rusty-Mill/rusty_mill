...
//! no RFC 3986 edge cases): those are out of scope and can be added if a
//! real caller needs them.
//!
//! Ported near-verbatim from `rusty_request`'s `url.rs` (donor 2 of the
//! `rusty_http` mission handoff) -- the one change is that
//! `absolute_form`/`resolve_redirect` are `pub` here rather than
//! `pub(crate)`, since this crate has consumers outside itself.

use crate::error::{Error, Result};

/// A parsed `http(s)://host[:port]/path?query` URL.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Url {
    /// Lowercased; `"http"` or `"https"`.
    pub scheme: String,
    /// The bare host, exactly as written (no case normalization).
    pub host: String,
    /// The port, defaulted from the scheme (80/443) when not written.
    pub port: u16,
    /// Always starts with `/`.
    pub path: String,
...
impl Url {
    /// Parses an `http(s)://host[:port]/path?query` string.

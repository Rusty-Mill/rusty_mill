...
use crate::wire::{
    read_array_len, read_checked_array_len, read_i16, read_i32, read_i64, read_string, write_i16,
    write_i32, write_i64, write_string,
};
...
impl ListOffsetsResponse {
    ...
    pub fn decode(reader: &mut Reader) -> Result<Self, CodecError> {
        const LIST_OFFSETS_TOPIC_MIN_LEN: usize = 2 + 4; // empty name + partition_count
        let topic_count = read_checked_array_len(reader, LIST_OFFSETS_TOPIC_MIN_LEN)?;
        let mut topics = Vec::with_capacity(topic_count);
        for _ in 0..topic_count {
            let name = read_string(reader)?;
            const LIST_OFFSETS_PARTITION_MIN_LEN: usize = 4 + 2 + 8 + 8; // partition_index + error_code + timestamp + offset
            let partition_count = read_checked_array_len(reader, LIST_OFFSETS_PARTITION_MIN_LEN)?;
...
mod tests {
    ...
    fn decode_rejects_a_huge_topic_count_from_a_tiny_buffer() {
        ...
        let bytes = writer.into_vec();
        let mut reader = Reader::new(&bytes);
        let err = ListOffsetsResponse::decode(&mut reader).unwrap_err();
        assert!(matches!(
            err,

...
| `aisf_stage` | `model` = **AISF stage name** (`"triage"` or `"validation"`), `aisf_binary_path` = path to AISF's built `software-factory` binary | Not a chat API — drives a real, governed, multi-turn agent stage via AISF's `eval-stage` subcommand as a subprocess per rollout. Only for `executor`; `optimizer`/`reflector` should stay plain chat backends. `configs/aisf_triage_example.yaml` / `configs/aisf_validation_example.yaml`. Both `triage` and `validation` now have a `claude_cli`/MCP-bridge driver on AISF's side, so neither needs a real `ANTHROPIC_API_KEY` to run live — see README's `aisf_stage` section. For a full `train` run with `optimizer`/`reflector` also on `claude_cli` (zero API keys anywhere), see `configs/aisf_triage_claude_cli_example.yaml`. |
